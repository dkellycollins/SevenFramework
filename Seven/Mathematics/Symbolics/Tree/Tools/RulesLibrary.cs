...
using Seven.Mathematics.Symbolics.RulesCollection;
    ...
    public static class RulesLibrary
    {
        ...
        public static List<Rule> GetSimplificationRules()
        {
            var simplificationRules = AlgebraicRules.Get()
                                 .Where(r => r.Tags.Contains(StdTag.Simplification))
                                 .ToList();
            simplificationRules.AddRange(LogicRules.Get().Where(r => r.Tags.Contains(StdTag.Simplification)));
